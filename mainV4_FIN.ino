...
DayAlarms mondayAlarms = {};
DayAlarms tuesdayAlarms = {};
DayAlarms wednesdayAlarms = {};
DayAlarms thursdayAlarms = {};
DayAlarms fridayAlarms = {};
DayAlarms saturdayAlarms = {};
DayAlarms sundayAlarms = {};
...
const int buzzerPin = 26;  // GPIO26 for buzzer/manual control
...
void handleRoot() {
  ...
  if (server.hasArg("COPY_ALARMS")) {
    ...
    // Save alarms to flash after copying
    saveAlarmsToFlash();
  ...
  if (server.hasArg("DAY_SELECTED")) {
    ...
    // Save alarms to flash after updating
    saveAlarmsToFlash();
  ...
  if (server.hasArg("LED_ON")) {
    digitalWrite(buzzerPin, LOW); // Turn ON buzzer/appliance
    server.send(200, "text/html", APPLIANCES_ON);
    return;
  }
  
  if (server.hasArg("LED_OFF")) {
    digitalWrite(buzzerPin, HIGH); // Turn OFF buzzer/appliance
...
void setup(void) {
  ...
  // Initialize pin (remove duplicate Man_BUZ pinMode)
  pinMode(buzzerPin, OUTPUT);
  digitalWrite(buzzerPin, HIGH);

  // Initialize RTC with retries and error handling
  ...
  for(int i=0; i<5; i++) {
    if (rtc.begin()) {
      ...
      Serial.print("RTC not found, retrying... Attempt ");
      Serial.println(i+1);
      delay(1000);
    }
  }
  rtcInitialized = rtcStarted;
  if (!rtcStarted) {
    Serial.println("ERROR: Couldn't find RTC after 5 attempts. Check wiring and power!");
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("RTC ERROR!");
    lcd.setCursor(0, 1);
...
void loop(void) {
  ...
  // Efficient LCD update: only update if time or date changes (excluding seconds)
  static int lastHor = -1, lastMin = -1;
  static int lastDay = -1, lastMonth = -1, lastYear = -1;
  if (Hor != lastHor || Min != lastMin || p_day != lastDay || p_month != lastMonth || p_year != lastYear) {
    lastHor = Hor;
    lastMin = Min;
    lastDay = p_day;
    lastMonth = p_month;
    ...
    // Update LCD display (no clear, just overwrite)
    lcd.setCursor(0, 0);
    lcd.print("Time: ");
    lcd.print(Hor);
    lcd.print(":");
